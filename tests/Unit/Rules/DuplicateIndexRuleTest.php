...
it('reports a foreign key declared more than once on the same column', function (): void {
    $orders = TableSchema::make('orders');
    $orders->addForeignKey(new ForeignKey(columns: 'customer_id', referencesTable: 'customers', referencesColumn: 'id', name: 'orders_customer_id_foreign'));
    $orders->addForeignKey(new ForeignKey(columns: 'customer_id', referencesTable: 'customers', referencesColumn: 'id', name: 'orders_customer_id_foreign_2'));
...
it('reports duplicates independently across multiple tables', function (): void {
    $orders = TableSchema::make('orders');
    $orders->addForeignKey(new ForeignKey(columns: 'customer_id', referencesTable: 'customers', referencesColumn: 'id', name: 'a'));
    $orders->addForeignKey(new ForeignKey(columns: 'customer_id', referencesTable: 'customers', referencesColumn: 'id', name: 'b'));

    $invoices = TableSchema::make('invoices');
    $invoices->addForeignKey(new ForeignKey(columns: 'order_id', referencesTable: 'orders', referencesColumn: 'id', name: 'c'));
